...
// Create new test
router.post('/', (req, res) => {
  const { title, description, client_id, test_date, status, tags } = req.body;
  ...
  db.run(
    `INSERT INTO tests (title, description, client_id, test_date, status)
     VALUES (?, ?, ?, ?, ?)`,
    [title, description, client_id, test_date, status || 'pending'],
    function(err) {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
...
router.put('/:id', (req, res) => {
  ...
  db.run(
    `UPDATE tests
     SET title = ?, description = ?, client_id = ?, test_date = ?, status = ?, updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    [title, description, client_id, test_date, status, id],
    function(err) {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
